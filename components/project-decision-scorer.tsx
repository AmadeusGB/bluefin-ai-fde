...
type Answer = 0 | 1 | 2;
type Question = { text: string; critical?: boolean };
const groups: Array<{ key: string; label: string; questions: Question[] }> = [
  {
    key: "value",
    label: "问题与价值",
    questions: [
      { text: "问题持续发生，并能用时间、收入、成本或风险描述损失" },
      { text: "成功可以用一到两个业务指标和当前基线核验" },
      { text: "首个范围可以在一个流程、团队或产品线内闭环" },
      { text: "即使不使用 AI，这个问题也值得企业投入解决" },
    ],
  },
  {
    key: "evidence",
    label: "数据与证据",
    questions: [
      { text: "能提供代表性的真实数据、工作样本或历史记录" },
      {
        text: "数据来源、使用授权、敏感范围和保存方式已经明确",
        critical: true,
      },
      { text: "可以建立测试集，并区分正确、错误与不可回答" },
      { text: "结果能够写回原工作流并留下可追溯记录" },
    ],
  },
  {
    key: "owner",
    label: "负责人和采用",
    questions: [
      { text: "有一名业务负责人对结果、资源和跨部门协调负责", critical: true },
      { text: "一线使用者能够参与诊断、试用、反馈和验收" },
      { text: "上线后有人负责知识、规则、评估集与异常处理" },
      { text: "管理者接受从小范围验证，而不是一次覆盖全集团" },
    ],
  },
  {
    key: "risk",
    ...
    questions: [
      {
        text: "高风险输出、价格、承诺、审批和执行动作保留人工闸门",
        critical: true,
      },
      { text: "系统错误时可以拒答、回退、暂停并恢复到原流程" },
      { text: "访问权限、操作日志、监控告警和责任边界可以落实" },
      { text: "已经定义 30 / 60 / 90 天复查与停止条件" },
    ],
  },
];
...
const options: Array<{ value: Answer; label: string; detail: string }> = [
  { value: 2, label: "已具备", detail: "有事实或材料可证明" },
  { value: 1, label: "部分具备", detail: "方向明确但仍需补齐" },
  { value: 0, label: "尚未具备", detail: "目前没有可核验证据" },
];
...
export function ProjectDecisionScorer() {
  ...
  const criticalFailures = questions.flatMap((question, index) =>
    question.critical && answers[index] === 0 ? [question.text] : [],
  );
  const dimensions = groups.map((group) => {
    const indexes = questions.flatMap((question, index) =>
      question.group === group.key ? [index] : [],
    );
    const points = indexes.reduce<number>(
  ...
  const decision = useMemo(() => {
    if (answered < questions.length)
      return ["待完成", "完成全部 16 项后生成项目决策。"];
    if (criticalFailures.length >= 2 || score < 30)
      return [
        "STOP",
        "存在多个生产前提缺失。停止开发，重新定义问题或取消当前项目。",
      ];
    if (criticalFailures.length === 1 || score < 50)
      return ["HOLD", "关键条件尚未成立。暂停开发，先解决红线问题并补齐证据。"];
    if (score < 75)
      return [
        "ADJUST",
        "方向可能成立，但需要收窄范围并补齐最低维度后再进入 MVD。",
      ];
    return ["GO", "主要前提已具备，可以进入资格确认并核验首个 MVD 边界。"];
  }, [answered, score, criticalFailures.length]);
      ...
      <aside className="self-start bg-[#071817] p-8 text-white lg:sticky lg:top-24 print:static">
        ...
        <div className="mt-7 border-t border-white/15 pt-6">
          ...
          {criticalFailures.length > 0 && (
            <div className="mt-5 border border-[#ff9b8a]/40 p-4 text-sm text-[#ffb4a8]">
      ...
      <div>
          ...
          <section key={group.key} className="mb-12">
            ...
            <div className="space-y-5">
              {group.questions.map((question, questionIndex) => {
                const index = groupIndex * 4 + questionIndex;
                return (
                  <fieldset
                    key={question.text}
        ...
        <div className="border-t border-foreground/20 pt-7">
          <p className="text-sm leading-6 text-muted-foreground">
            已完成 {answered}/{questions.length} 项。评分规则公开透明：已具备 2
            分、部分具备 1 分、尚未具备 0 分；关键红线会降低决策等级。
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Button
              ...
              render={
                <Link
                  href={`/apply?score=${score}&decision=${decision[0]}&source=project-scorer`}
                />
              }
              className="h-12 rounded-none"
            >
              提交报告，申请资格确认 <ArrowRight />
            </Button>
            <Button
              variant="outline"
              onClick={copy}
