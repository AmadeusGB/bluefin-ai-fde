  ...
  return (
    <><SiteHeader/><main className="min-h-screen bg-background text-foreground">
      <section className="mx-auto grid min-h-[calc(100vh-72px)] max-w-[1500px] grid-cols-1 gap-16 px-5 pb-10 pt-16 lg:grid-cols-[1.35fr_.65fr] lg:items-center lg:px-10">
        <div><p className="mb-7 flex items-center gap-3 text-xs font-bold uppercase tracking-[.16em]"><span className="size-2 rounded-full bg-[#17856c] shadow-[0_0_0_6px_rgba(23,133,108,.12)]" />企业 AI 落地 · Forward Deployed Engineering</p><h1 className="max-w-5xl text-[clamp(3.7rem,8vw,8.7rem)] font-black leading-[.92] tracking-[-.075em]">别再做<br/><em className="not-italic text-[#147e66]">“看起来能用”</em><br/>的 AI。</h1><p className="mt-9 max-w-3xl text-lg leading-8 text-muted-foreground lg:text-xl">蓝旗鱼进入真实业务现场，找到最贵的问题，用真实数据跑通最小可行部署，并把结果沉淀成企业能够长期使用的系统与能力。</p><div className="mt-10 flex flex-wrap items-center gap-6"><Button nativeButton={false} render={<Link href="/diagnostic" />} size="lg" className="h-13 rounded-none px-6 text-base font-bold">申请业务诊断 <ArrowRight /></Button><Link href="/fde" className="group flex items-center gap-2 font-bold">先了解 FDE <MoveDownRight className="transition-transform group-hover:translate-x-1" /></Link></div></div>
      ...
      <section className="bg-white px-5 py-24 lg:px-10"><div className="mx-auto max-w-[1500px]"><div className="grid gap-10 lg:grid-cols-2"><div><p className="eyebrow text-[#147e66]">在线 FDE 工具</p><h2 className="mt-5 text-5xl font-black tracking-[-.055em] lg:text-7xl">把模糊需求，变成可判断的项目。</h2></div><p className="self-end text-lg leading-8 text-muted-foreground">先完成适配度评估和五张地图，再用 MVD 设计器写清楚真实数据、人机边界与成功条件。工具本身免费，结论不替代现场核验。</p></div><div className="mt-14 grid md:grid-cols-3">{[['适配度评估','四个维度、12 项条件，生成初步报告。','/diagnostic'],['五张地图','问题、流程、数据、责任与风险。','/tools/five-maps'],['MVD 设计器','把最小可行部署写成一页草案。','/tools/mvd-designer']].map(([t,d,h],i)=><Link key={t} href={h} className="group border-t border-foreground/20 py-7 md:border-r md:px-7 first:pl-0"><span className="text-xs text-[#147e66]">0{i+1}</span><h3 className="mt-10 text-2xl font-black">{t}</h3><p className="mt-3 leading-7 text-muted-foreground">{d}</p><span className="mt-6 inline-flex items-center gap-2 font-bold">立即使用 <ArrowRight className="size-4 transition-transform group-hover:translate-x-1"/></span></Link>)}</div></div></section>
      <section className="bg-[#dff6e6] px-5 py-24 lg:px-10"><div className="mx-auto grid max-w-[1500px] gap-12 lg:grid-cols-2"><div><p className="eyebrow text-[#147e66]">合作边界</p><h2 className="mt-5 text-5xl font-black tracking-[-.055em] lg:text-7xl">先证明值得做，<br/>再开始开发。</h2></div><div className="space-y-4 text-lg font-semibold">{['有真实数据、真实流程和明确负责人','允许工程师进入现场，让一线参与验证','愿意从一个高价值问题开始','接受人工审核、风险护栏与结果复查'].map(x => <p key={x} className="flex gap-4 border-t border-foreground/20 py-4"><Check className="text-[#147e66]" />{x}</p>)}<Button nativeButton={false} render={<Link href="/diagnostic" />} size="lg" className="mt-5 h-13 rounded-none bg-[#ff735d] px-6 text-[#071817] hover:bg-[#ff826f]">5 分钟适配度自检 <ArrowRight /></Button></div></div></section>
    </main><SiteFooter/></>
  );
}
