...
export default function Page(){return <><SiteHeader/><main><PageHero eyebrow="公开标准 · 版本 1.0" title="案例不是故事。案例是一组可核验证据。" intro="所有蓝旗鱼案例统一公开项目背景、原始流程、基线、数据范围、MVD 边界、结果、风险和客户接管方式。没有授权或数据时，必须明确标记为脱敏案例、内部实践、演示原型或待验证假设。"/><section className="px-5 py-20 lg:px-10"><div className="mx-auto max-w-[1200px]"><div className="grid gap-x-10 md:grid-cols-2">{sections.map(([n,t,d])=><article key={n} className="border-t border-foreground/20 py-7"><span className="text-xs font-bold text-[#3657d6]">{n}</span><h2 className="mt-3 text-2xl font-black">{t}</h2><p className="mt-3 leading-7 text-muted-foreground">{d}</p></article>)}</div><div className="mt-14 bg-[#0b1238] p-8 text-white lg:p-12"><p className="eyebrow text-[#cdd5ff]">发布底线</p><h2 className="mt-4 text-3xl font-black lg:text-5xl">不把原型包装成成功，<br/>不把相关性写成因果。</h2><p className="mt-5 max-w-3xl leading-8 text-white/60">每项结果都要说明口径、样本、时间范围和限制；无法核验的客户评价只能作为证言，不能替代业务结果。</p></div></div></section></main><SiteFooter/></>}
