...
export async function POST(request:Request){
  try{
    ...
    const decision=['GO','ADJUST','HOLD'].includes(clean(body.decision,12))?clean(body.decision,12):null;
    ...
    await getD1().prepare(`INSERT INTO diagnostic_applications (id,created_at,name,company,contact,role,industry,problem,diagnostic_score,decision,source,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id,Date.now(),name,company,contact,role,industry,problem,diagnosticScore,decision,'website','new').run();
